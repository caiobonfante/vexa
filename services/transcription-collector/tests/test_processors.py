"""Unit tests for streaming/processors.py.
...
def _make_transcription_payload(segments=None, token="valid.token.here", msg_type="transcription", uid="sess-1"):
    """Helper to build a stream message payload."""
    data = {
        "type": msg_type,
        "token": token,
        "platform": "teams",
...
def _make_transcript_bundle(speaker="Alice", confirmed=None, pending=None, token="valid.token.here", uid="sess-1"):
    """Helper to build a transcript bundle payload."""
    data = {
        "type": "transcript",
        "token": token,
        "speaker": speaker,
...
class TestProcessSpeakerEventMessage:
    async def test_valid_event_stored_in_redis(self, mock_redis):
        from streaming.processors import process_speaker_event_message
    ...
    async def test_missing_required_fields_returns_true(self, mock_redis):
        ...
        del event  # oops, let me rebuild without uid
        event = {"relative_client_timestamp_ms": "1000", "event_type": "start", "participant_name": "Bob"}
        result = await process_speaker_event_message("msg-1", event, mock_redis)
    ...
    async def test_redis_error_returns_false(self, mock_redis):
        ...
        assert result is False  # Recoverable error, do not ack
...
class TestProcessTranscriptBundle:
    async def test_confirmed_segments_stored(self, mock_redis):
        from streaming.processors import process_transcript_bundle
    ...
    async def test_confirmed_with_empty_text_skipped(self, mock_redis):
        ...
        confirmed = [
            {"segment_id": "seg-1", "text": "   ", "start": 0.0, "end": 1.0},  # empty text
...
class TestProcessStreamMessage:
    async def test_missing_payload_returns_true(self, mock_redis):
        from streaming.processors import process_stream_message
    ...
    async def test_invalid_json_payload_returns_true(self, mock_redis):
        ...
        assert result is True  # Bad JSON is acked to avoid retry loop

    async def test_unknown_message_type_returns_true(self, mock_redis):
        from streaming.processors import process_stream_message

        payload = json.dumps({"type": "unknown_type", "token": "t"})
        # verify_meeting_token must return valid claims for the message to proceed
        with patch("streaming.processors.verify_meeting_token", return_value={"meeting_id": 1, "platform": "teams", "native_meeting_id": "abc"}):
            with patch("streaming.processors.async_session_local") as mock_session_ctx:
                mock_db = AsyncMock()
    ...
    async def test_failed_token_verification_returns_true(self, mock_redis):
        ...
        with patch("streaming.processors.verify_meeting_token", return_value=None):
            with patch("streaming.processors.async_session_local") as mock_session_ctx:
                mock_db = AsyncMock()
                mock_session_ctx.return_value.__aenter__ = AsyncMock(return_value=mock_db)
                mock_session_ctx.return_value.__aexit__ = AsyncMock(return_value=False)
        ...
        assert result is True  # Invalid token acked, not retried
