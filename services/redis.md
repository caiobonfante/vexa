...
1. **Streams** — durable, ordered message delivery for transcription segments. WhisperLive produces segments, transcription-collector consumes them. If the collector is slow or restarts, the stream retains messages until they're consumed. This is why transcripts don't get lost.
...
Redis 7 (Alpine), single instance, no persistence configuration (data is ephemeral by design — the source of truth for transcripts is Postgres, for billing is Stripe).

### Data flows

```
WhisperLive ──XADD──► Redis Stream (transcription_segments) ──XREADGROUP──► transcription-collector ──► Postgres
                       │
                       └──XADD──► Redis Stream (speaker_events) ──XREADGROUP──► transcription-collector

bot-manager ──PUBLISH──► Redis Pub/Sub (meeting:{id}:status) ──SUBSCRIBE──► api-gateway ──► WebSocket clients

bot-manager ──PUBLISH──► Redis Pub/Sub (bot:{uid}:command) ──SUBSCRIBE──► vexa-bot (via api-gateway proxy)
...
|---------|------|----------|----------|---------|
| `transcription_segments` | Stream | WhisperLive | transcription-collector | Transcript segments with text, timestamps, speaker info |
| `speaker_events` | Stream | WhisperLive | transcription-collector | Speaker activity events (join, leave, speaking) |
...
| `bot:{uid}:command` | Pub/Sub | bot-manager | api-gateway → bot WebSocket | Bot control commands (stop, reconfigure, speak, chat) |
...
- Stream producer: [`services/WhisperLive/whisper_live/server.py`](WhisperLive/whisper_live/server.py) — `TranscriptionCollectorClient.xadd()`
