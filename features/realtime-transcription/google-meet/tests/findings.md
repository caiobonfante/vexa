...
| Speaker identity locks (human) | 50 | **dkn-pwrq-obk (2026-03-27).** Dmitriy identified correctly, but only 1 human tested. Prior meeting 21: Dmitriy Grankin correctly identified (2/2 segments). Human locking works but sample size too small for high confidence. | 2026-03-27 | Test with 3+ human speakers in natural conversation; **CSRC upgrade** for instant identity |
| Multi-track dedup | 70 | **3-speaker collection (dkn-pwrq-obk).** 3 speakers on 3 tracks, no duplicate content observed. Speaker name bug fix (9e1c774a) ensures locked names propagate to segments. Prior meeting 21's dedup issue was actually the name-not-applied bug, now fixed. | 2026-03-27 | Test with 5+ speakers (SFU remapping); **CSRC upgrade** for instant identity during remaps |
...
| Confirmation logic | 80 | **Re-confirmed (dkn-pwrq-obk).** Confirm latency 6-7s (improved from 11.8s in meeting 21). Confirmation triggered reliably for all scripted utterances. LocalAgreement-2 prefix algorithm working correctly. | 2026-03-27 | Run replay with fresh data; test with overlapping speech |
...
**Overall: 88/100** — 3-speaker collection (dkn-pwrq-obk, 2026-03-27). 3 TTS bots (Alice/Bob/Charlie) + 1 human (Dmitriy) in real Google Meet. **100% speaker accuracy** (3/3 correct), **100% completeness** (every scripted utterance transcribed), **WER 18%** (number normalization only), **confirm latency 6-7s**. Full pipeline verified: bot → Whisper → Redis → Postgres → REST API. Speaker name bug (locked name not applied) fixed in commit 9e1c774a.

**To reach 90:** (1) Test with 3+ human speakers in natural conversation. (2) CSRC upgrade for instant identity without voting. (3) Test with 5+ speakers to validate SFU remapping dedup. (4) Test WS during active speech for live segment delivery.
