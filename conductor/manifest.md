...
graph TD
    ...
    Orch -->|read| Manifest[feature manifest.md]
...
```mermaid
graph LR
    M[manifest.md<br/>what SHOULD be true] -->|agent reads| Code[code changes<br/>what IS now true]
...
1. **manifest.md** is the design intent. Agent reads it before touching code.
...
- Why it exists, design intent → `manifest.md`
...
- Conductor MUST pass the feature's manifest.md to the orchestrator via `--append-system-prompt-file`
