...
| **TC fold** | 7/10 | Redis Stream consumer with consumer groups, stale message claiming, UPSERT by `segment_id`, background task lifecycle management in `main.py` | Issuer mismatch fixed (2026-03-29): accepts both `meeting-api` and `bot-manager` issuers. Connection retry is basic (sleep + retry loop, no backoff) |
...
1. **MeetingToken issuer mismatch** (fixed 2026-03-29): `verify_meeting_token()` in `collector/processors.py` now accepts both `iss: "meeting-api"` and `iss: "bot-manager"`. The legacy bot-manager service (still running from cached Docker image) mints tokens with `iss: "bot-manager"`. Once bot-manager is fully removed and all bots are created through meeting-api, the `"bot-manager"` issuer can be dropped.
