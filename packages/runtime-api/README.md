...
Audited 2026-03-27. 5029 lines across 13 source files + 8 test files. Unit test run: **59 passed, 7 failed, 1 collection error** (test_api.py fails to import on Python <3.10 due to `X | None` syntax).
...
| Test coverage | 40/100 | 66 tests collected, 59 pass. Tests cover: state CRUD, profile loading, lifecycle idle/callback/reconcile, scheduler CRUD/retry/cron, backend ABC, process backend start/stop/inspect | 7 tests fail: 2 `count_user_containers` (missing function), 1 `process_stop` (race condition), 1 `idle_loop` (timing-dependent), 3 profile tests (global state leaks between tests). `test_api.py` won't collect on Python 3.9 (`dict[str, str] | None` syntax). `fakeredis` not in `pyproject.toml` dev deps. Zero Docker backend tests against real Docker API. Zero K8s backend tests (only dataclass field assertions). FakeRedis mocks ignore `ex=` TTL parameter — TTL behavior untested. |
...
| Standalone (`make up && make test`) | 35/100 | `Makefile` has `up`, `test-unit`, `test-integration` targets. `make up` waits for health with 30s timeout | `make test-unit` will fail: `fakeredis` not in deps, 7 test failures, collection error on Python <3.10. `make test-integration` requires Docker daemon access and pulling images. CI workflow references `ORCHESTRATOR_BACKEND=docker` for integration tests but service container won't have Docker socket access. |
| Security | 25/100 | API key middleware exists (optional via `API_KEYS` env). Health/docs endpoints skip auth | Auth disabled by default (empty `API_KEYS`). No input validation on container names — user-supplied `name` goes directly into Docker API URLs (`/containers/create?name={spec.name}`) and Redis keys. CORS defaults to `*`. No rate limiting. `/exec` endpoint allows arbitrary command execution with no restrictions. `callback_url` accepts any URL including internal network addresses (SSRF vector). |
...
| Documentation | 65/100 | README is thorough: architecture diagram, module map, env var table, backend comparison matrix, honest resource limits table, lifecycle flow diagrams | README claims per-user concurrency enforcement — not implemented. Idle management description says `HGET container:{name} last_touch` — actual implementation uses `updated_at` field via `set_container`. Scheduler module undocumented. `profiles.example.yaml` has `max_per_user` field that does nothing. |
...
- **`max_per_user` profile field is decorative** — read from YAML, never checked during container creation
...
- **`fakeredis` missing from dev deps** — `test_api.py` imports it but `pyproject.toml` doesn't list it
...
- **No container name sanitization** — names go directly into URLs and Redis keys
- **SSRF via `callback_url`** — no URL validation, can target internal services
