...
| 2026-03-30 | Gotcha G7: convention changes require full-codebase consumer search | Agent updated compose, Helm values, Makefiles, and READMEs for a new image tagging convention (vexaai/ prefix, YYMMDD-HHMM timestamps). Updated 10 planned files. Missed 14 stale references across 9 unplanned files: Python config defaults (meeting_api/config.py `vexa-bot:latest`, agent_api/config.py `agent:latest`), feature deploy compose files (agentic-runtime `vexa-bot:dev`, `vexa-agent:dev`), env templates (env-example, feature .env files), bot scripts (run-zoom-bot.sh, hot-debug.sh). **Root assumption that failed: "the plan covers all the files that matter."** Convention changes have unbounded blast radius — every file that hardcodes a default value is a consumer. The fix: run a full-codebase search for the OLD convention (all patterns) BEFORE declaring refactoring done, not as a post-delivery cleanup. This generalizes G6 (search all case variants) to: search all files, not just the ones you planned to touch. |

| 2026-03-31 | Gotcha G8: confidence must reflect the critical path, not the easy tests | Agent claimed 75/100 on bot lifecycle refactor. 10/15 DoD items passed via API-level curl. But the 5 untested items were the ones that could ONLY be validated with real meetings (admission detection, timeouts, full lifecycle, dashboard). When tested: bot falsely reported `active` while sitting in the waiting room. Host Playwright screenshot confirmed "Admit 1 guest" visible. Agent had trusted bot self-reports as evidence instead of using available CDP screenshots. **Root assumptions that failed:** (1) "10/15 items = 75%" — confidence was weighted by count not risk; (2) "bot logs saying admitted = evidence" — should have used Playwright CDP screenshot as source of truth. **New pattern: weighted DoD.** Each DoD item gets a risk weight. API-level tests are low-weight (3-5pts). Live meeting tests requiring CDP screenshot evidence are high-weight (15pts). Confidence = weighted sum / total. This makes it impossible to reach 70% without testing the critical path. |
| 2026-03-31 | Research: weighted DoD with ceiling-based confidence | Cross-domain research across 6 areas confirms ceiling mechanic as best-in-class. Found independently in: Kayenta canary analysis (`critical: true` bypasses averaging → score 0), IEC 62304 ("highest class of any component rules all"), SonarQube quality gates (binary pass/fail, no partial credit), SIL/IEC 61508 (hard prerequisites per safety level), Google Test Certified (leveled gates with peer validation). Formula: `confidence = min(weighted_score, lowest_ceiling_of_failing_items)`. Makes it mathematically impossible to game past critical path. Evidence quality scaling validated by mutation testing research (mutation score correlates with real fault detection, R=0.87) and AI calibration research (agents predict 73% success vs 35% actual — Kaddour 2026). Full research: `.claude/research/weighted-dod-confidence.md` |
