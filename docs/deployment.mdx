...
### 2. Two transcription modes

| Mode | What you need | How |
|------|--------------|-----|
| **Default** | External transcription service (no GPU) | Set `TRANSCRIPTION_SERVICE_URL` in `.env` |
| **Local GPU** | NVIDIA GPU + drivers | Set `LOCAL_TRANSCRIPTION=true` in `.env` |
...
### 5. Vexa Lite - For Users (Production Deployment)
