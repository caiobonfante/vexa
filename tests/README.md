...
Deploy modes also have validation tests:
```
deploy/
├── compose/tests/test_compose_config.sh   # Validates docker-compose YAML and service definitions
├── lite/tests/test_dockerfile.sh          # Validates Dockerfile.lite structure and dependencies
└── helm/tests/test_helm_lint.sh           # Validates Helm chart structure and runs helm lint
```
...
```bash
bash deploy/compose/tests/test_compose_config.sh
bash deploy/lite/tests/test_dockerfile.sh
